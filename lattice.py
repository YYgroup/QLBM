...
class qD2Q9(D2Q9):
    ...
    def controlled_addminus(qc, lowerindex, higherindex, ctrl_str=None, ctrl_qubit=None, type=1):
        ...
        for iiii in range(higherindex, lowerindex - 1, -1):
            qubitlist = (iiii - lowerindex) * [type]
    ...
    def quantum_transport(self, qc, step, in_anc, m_th, index=0):
        qx = int(np.log2(self.nx))
        qy = int(np.log2(self.ny))
        in_anc = qx + qy + 9
        ...
        if index == 0:
            t_list = range(512)
        for ii in t_list:
            spin_index = format(ii, '09b')
            m = spin_index.count('1')
            if m > m_th or m == 0:
                continue
            bit8, bit7, bit6, bit5, bit4, bit3, bit2, bit1, bit0 = (int(spin_index[i]) for i in range(9))
            bitlist = [bit8, bit7, bit6, bit5, bit4, bit3, bit2, bit1, bit0]
            bitlist.reverse()
            qc.reset(in_anc)
            qc.reset(in_anc + 1)
            qc.reset(in_anc + 2)
